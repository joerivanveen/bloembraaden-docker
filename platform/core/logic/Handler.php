...
class Handler extends BaseLogic
{
    ...
    {
        parent::__construct();
        header('X-Powered-By: Bloembraaden.io', true);
    ...
    public function Act()
    {
        if (null === ($action = $this->resolver->getAction())) return null;
        ...
        if ($action === 'javascript') {
            ...
            $file_location = CORE . 'presentation/cache/' . Setup::$instance_id . '-' . $version . '.js.gz';
            ...
            $doc .= file_get_contents(CORE . '../htdocs/client/peat.js');
            ...
            if (false === file_exists($file_location)) file_put_contents($file_location, $response, 2);
            header('Cache-Control: max-age=2592000'); //30days (60sec * 60min * 24hours * 30days)
            header('Content-Type: text/javascript');
            header('Content-Encoding: gzip');
            header('Content-Length: ' . strlen($response));
            ...
            // @since 0.7.6 get cached version when available for non-admins
            $file_location = CORE . 'presentation/cache/' . Setup::$instance_id . '-' . $version . '.css.gz';
            if (false === ADMIN and true === file_exists($file_location)) {
                $response = file_get_contents($file_location);
                header('Cache-Control: max-age=2592000'); //30days (60sec * 60min * 24hours * 30days)
            ...
            $doc = file_get_contents(CORE . '../htdocs/client/peat.css');
        ...
        } elseif ($action === 'suggest_titles') {
            if (isset(($props = $this->resolver->getProperties())['terms'])) {
                $src = new Search();
                $out = $src->suggestTitles($props['terms']);
                $src = null;
            }
            ...
            if (isset($_GET['id']) && ($psp = $instance->getPaymentServiceProvider())) {
            ...
            if ('get_template' === $action) {
                ...
                if ('update_element' === $action) {
                    ...
                    if (isset($post_data->instance_id) and $admin->isRelatedInstanceId(($instance_id = $post_data->instance_id))) {
                        ...
                        $file_name = Help::slugify($this->getSession()->getInstance()->getName()) . '-Templates.json';
                    ...
                    $instance_id = ($post_data->type === 'instance') ? $post_data->id : Setup::$instance_id;
                    ...
                    if (false === isset($post_data->instance_id)) {
                        ...
                        $file_location = CORE . 'presentation/cache/' . $instance_id . '.css';
                        $doc = file_get_contents(CORE . '../htdocs/client/peat.css'); // reset and some basic stuff
                        ...
                        if (false === file_put_contents($file_location, $doc, 2)) {
                            $this->addMessage(__('Could not save css', 'peatcms'), 'warn');
                        }
                    ...
                    if (isset($post_data->order_id)) {
                        if (($psp = $instance->getPaymentServiceProvider())) {
                            return $psp->capturePayment((int)$post_data->order_id);
                        }
                    }
    ...
    public function View()
    {
        ...
        if (($check_timestamp = $_SERVER['HTTP_X_CACHE_TIMESTAMP'] ?? null)) {
            ...
            if (true === $this->getDB()->cachex($slug, $check_timestamp)) {
                ...
                header('Content-Type: application/json');
                header('Content-Length: ' . strlen($response));
                header('X-Ecution-Time: ' . (microtime(true) - $_SERVER['REQUEST_TIME_FLOAT']));
                echo $response;
                die();
            }
        }
        ...
        $base_element = (($slug = $out->__ref ?? null)) ? $out->slugs->$slug : $out;
        ...
        if (defined('OUTPUT_JSON')) {
            ...
            $out->csrf_token = $this->getSession()->getValue('csrf_token'); // necessary for login page at the moment
            $out->nonce = Help::randomString(20);
            ...
            $out->dark_mode = $this->getSession()->getValue('dark_mode');
            // render in template
            $temp = new Template();
            ...
            $temp->renderGlobalsOnce(array(
                ...
                'session' => $this->getSession()->getVars(),
                'slug' => $out,
            ...
            $csp = "frame-ancestors 'none';default-src 'self' https://player.vimeo.com https://www.youtube-nocookie.com https://www.youtube.com https://stagconnect.acehubpaymentservices.com https://connect.acehubpaymentservices.com https://www.google.com; script-src 'self' 'nonce-$out->nonce'; connect-src 'self' https://www.google-analytics.com; img-src 'self' https://*.cdninstagram.com www.googletagmanager.com https://www.google-analytics.com data:;font-src 'self' https://fonts.gstatic.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com;base-uri 'self';form-action 'self';";
            ...
            if (ob_get_length()) { // false or 0 when there's no content in it
                ...
                $response = gzencode($temp->getHtml(), 9);
                unset($temp);
