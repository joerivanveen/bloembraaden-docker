...
class Instance extends BaseLogic
{
     ...
     */
    public
    function getPaymentServiceProvider(): ?PaymentServiceProvider
    {
...
// global stuff accessible from outside
    public
    function getPresentationInstance(): string
    {
        return (string)$this->row->presentation_instance;
    }

    public
    function getPresentationAdmin(): string
    {
     ...
     */
    public
    function getSetting(string $which)
    {
        return $this->row->$which ?? null;
    }

    public
    function isParked(): bool
    {
    ...
    function load(string $domain)
    {
        ...
        if (!($this->row = $this->getDB()->fetchInstance($domain))) {
            ...
            if (($canonical = $this->getDB()->fetchInstanceCanonicalDomain($domain))) {
                ...
                $this->handleErrorAndStop("No instance found for domain ‘{$domain}’",
                    __("Could not startup Bloembraaden", "peatcms"));
            }
        }
    }
}