...
class BaseLogic extends Base
{
    ...
    public function getDB(): DB
    {
        if (isset($this->db)) {
            ...
            /*if ($this->db->hasError()) {
                $this->handleErrorAndStop($this->db->getLastError(), __('Connecting to database failed', 'peatcms'));
            }*/
    ...
    public function getOutput(): \stdClass
    {
        ...
        if (!$this->completedRowForOutput) {
            ...
            // fields excerpt, description and content need to be parsed
            $parser = new Parser();
    ...
    public function cacheOutputObject(bool $returnOutputObject = true): \stdClass
    {
        ...
        if (isset($out->__ref) and false === strpos($out->__ref, '__')) {
            ...
            if (0 === $this->getResultCount()) {
                ...
                return ($returnOutputObject) ? $this->getOutputObject() : new \stdClass;
            }
            ...
            if ($returnOutputObject) $out = $this->getDB()->cached($slug, 1);
        }

        return (($returnOutputObject) ? $out : new \stdClass);
    }
