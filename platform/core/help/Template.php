...
class Template extends BaseLogic
{
    ...
    public function publish(): bool
    {
        ...
        if (true === $this->getDB()->updateColumns(
            ...
            // save it to disc: /presentation/[template_id].gz
            if (true === file_exists(CORE . 'presentation/templates/')) {
                if (false === file_put_contents(
                        CORE . 'presentation/templates/' . $this->getId() . '.gz', // json saved as gzipped string
                        \gzencode($json_prepared, 9) // compress as well
                    )) {
                    $this->addMessage(__('Could not write template to disk', 'peatcms'), 'error');
                ...
                $this->addMessage(sprintf(__('Folder ‘%s’ does not exist', 'peatcms'), 'presentation/templates/'), 'error');
    ...
    public function renderConsole(\stdClass $output)
    {
        ...
        $this->insertIntoHtml('<link rel="stylesheet" name="bloembraaden-admin-css" href="/client/admin.css?version=' . Setup::$VERSION . '">', false);
        // add config for editor
        $default_location = CORE . 'presentation/instance/editor_config.json';
        $custom_location = CORE . 'presentation/instance/' . Setup::$PRESENTATION_INSTANCE . '/editor_config.json';
            ...
            if (in_array($out_type = gettype($output_object), ['string', 'integer', 'boolean'])) {
                ...
                if (isset($template[$tag_name])) {
                    ...
                    foreach ($template[$tag_name] as $index => $sub_template) {
                        ...
                        $temp_remember_html = $sub_template['html'];
                        if (isset($output_object[0])) { // build rows if present
                            ...
                            if (isset($sub_template['__row__'])) { // only render rows when present in the template
                                // $sub_template contains ['__row__'] indexed array, for all the row parts in ['html']
                                // so render those separately..., and finally render the ['html']
                                ...
                                foreach ($sub_template['__row__'] as $template_index => $row_template) {
                                        ...
                                        if (is_int($row_index)) { // this is a row
                                            ...
                                            if ($row_index === 1) { // check this only the second time the row is processed
                                                if (($add_string = $this->renderOutput($obj, (array)$row_template)) === $build_rows) {
                                                    ...
                                                    $sub_template['html'] = $build_rows;//$temp_remember_html;
                                    ...
                                    $sub_template['html'] = str_replace('{{__row__[' . $template_index . ']}}', $build_rows, $sub_template['html']);
                                }
                            }
                        }
    ...
    private function prepare(string $html): array
    {
        ...
        if (false !== strpos($html, '</head>')) {
            ...
            if (file_exists($file_location)) {
                ...
                echo '</style>';
            } else { // todo you may probably remove this backwards compatibility sometime...
                echo '<link rel="stylesheet" name="bloembraaden-css" id="bloembraaden-css" href="__action__/stylesheet?version=';
                echo Setup::$VERSION;
                echo '-{{template_published}}">';
            }
            echo '<script name="bloembraaden-js" id="bloembraaden-js" src="/__action__/javascript?version=';
            echo Setup::$VERSION;
            echo '-{{template_published}}" async="async" defer="defer"></script>';
            echo '</head>';
            // replace the end of the head tag with these script links
    ...
    private function convertTagsRemaining(string $html): string
    {
        ...
        for ($index = 0; $index < 10; ++$index) {
            $search = '[' . $index . ']}}';
            if (($end = strpos($html, $search)) === false) break; // don't loop any further if the indexes became too high
