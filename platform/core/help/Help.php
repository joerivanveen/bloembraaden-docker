...
class Help
{
    ...
    public static function getMessagesFromJson(string $json): array
    {
        // TODO some checking?
     ...
     * @since 0.4.0
     */
    public static function trigger_error(string $message, $level = E_USER_NOTICE)
    {
     ...
     * @since 0.4.0
     */
    public static function trace(string $msg)
    {
        $GLOBALS['trace'][] = $msg;
    }
    ...
    public static function addToIndexedArray(array &$arr, $value)
    {
        if (is_array($arr)) {
            $arr[] = $value;
        } else {
     ...
     * @return string the path the file is saved under, or null when failure
     */
    public static function scanFileAndSaveWhenOk(string $temp_path): ?string
    {
        ...
        if (file_exists($new_path)) { // this should never happen
            return null; // TODO log this when it happens
        } else {
     ...
     * peatcms relies on database for lowercase conversion, which is better with utf-8
     *
    ...
    public static function upgrade(DB $db)
    {
        ...
        if (\strrpos($sql, '-- rebuild cache database')) {
            // no longer supported, because cache sits in main database @since 0.6.0
        }
        //
        try {
            $db->run($sql);
        ...
        $files = glob(CORE . '/data/cache/*.info.serialized'); // get all file names
    ...
    public static function install(DB $db)
    {
        ...
        if ($do === 1) {
            ...
            if (PHP_VERSION_ID < 70400) {
                die('Peatcms needs php version 7.4 or higher.');
            }
            ...
            // done, feedback to user
            echo(sprintf('Successfully installed peatcms version %s', $version));
            echo '<br/>' . PHP_EOL;
