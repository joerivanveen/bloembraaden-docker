...
class Resolver extends BaseLogic
{
    ...
    public function __construct(string $request_uri, int $instance_id)
    {
        ...
        $request_uri = $this->getDB()->toLower(urldecode($request_uri));
        ...
        if (isset($this->instructions['action'])) {
            ...
            $this->instructions['action'] = $this->post_data->action;
        }
        // for fileupload...
            ...
            switch ($instruction) {
                ...
                case 'order':
                    if ($session->isAdmin()) {
                        ...
                        $src = '%';
                        if (isset($this->getTerms()[0])) $src .= $this->getTerms()[0]; // look up by last characters
                        // multiple orders
