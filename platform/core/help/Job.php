...
if ('1' === $interval) { // interval should be '1'
    echo 'mail order confirmations:' . PHP_EOL;
        ...
        if (isset($row->order_number)) {
            ...
            if (true === $row->payment_confirmed_bool) {
                ...
                if (true === $row->create_invoice && false === file_exists($filename)) {
                    ...
                    // save the pdf
                    file_put_contents($filename, $result, 0);
    ...
    echo 'update ci_ai: ' . PHP_EOL;
    echo $db->jobSearchUpdateIndexColumn() . PHP_EOL;
    echo 'empty expired lockers: ';
    ...
    foreach ($rows as $index => $row) {
        ...
        $save_path = Setup::$INSTAGRAM->media_folder;
        echo $media_url . ' as ';
    ...
    foreach ($updated_user_ids as $user_id => $ok) {
        ...
        foreach ($feeds as $index => $specs) {
            ...
            $feed = $db->fetchInstagramMediaForFeed($specs->instance_id, $specs->instagram_username, $specs->instagram_hashtag, $specs->quantity);
...
} elseif ('hourly' === $interval) {
    // check all the js and css in the presentation cache, delete old ones
    // also refresh the json files for the filters that are present here
    $dir = new \DirectoryIterator(CORE . 'presentation/cache');
    foreach ($dir as $index => $fileinfo) {
        if (!$fileinfo->isDot()) {
            if ('serialized' === $fileinfo->getExtension()) {
                $filename = $fileinfo->getFilename();
                $pieces = explode('.', $filename);
                if (!($instance_id = (int)$pieces[1])) {
                    unlink($fileinfo->getPathname());
                    echo 'Deleted ' . $fileinfo->getFilename() . PHP_EOL;
                    continue;
                }
                // 8 = length of 'filter' + two dots, e.g. filename starts with filter.141.
                // -5 = remove .serialized from the end
                $path = urldecode(substr($filename, strlen($instance_id) + 8, -11));
                $src = new Search();
                $src->getRelevantPropertyValuesAndPrices($path, $instance_id, true);
                echo 'Refreshed ' . $path . PHP_EOL;
            } elseif ('gz' === $fileinfo->getExtension()) {
    ...
    echo 'clean template folder: ';
    echo $db->jobCleanTemplateFolder() . PHP_EOL;
    // @since 0.8.9
    echo 'remove old sessions and shoppinglists: ';
    ...
    echo 'remove orphaned shoppinglist rows (variants): ';
    echo $db->jobDeleteOrphanedShoppinglistVariants() . PHP_EOL;
    // @since 0.8.9
    echo 'remove orphaned session variables: ';
    ...
    echo 'refresh Instagram access token: '; // @since 0.7.2
...
if (Setup::$VERBOSE || 'hourly' === $interval || 'daily' === $interval) {
    error_log(sprintf(
