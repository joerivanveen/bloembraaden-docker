...
class Parser extends Base
{
    ...
    /**
     * Parser constructor, sets the flag for sub_parser ie when called recursively, to parse text inside other text,
     * the default is false, when true the parser won’t automatically add paragraphs for instance
     * @param bool $as_sub_parser default false @since 0.6.17 when the parser is called for text inside other text
     */
    public function __construct(bool $as_sub_parser = false, LoggerInterface $logger = null)
    {
        parent::__construct();
        $this->logger = $logger;
        $this->log('New parser (' . (($as_sub_parser) ? 'true' : 'false') . ')');
        $this->is_sub_parser = $as_sub_parser;
        $this->started = microtime(true);
    }

    /**
     * Wrapper for the logger so you can just call this and don’t bother about anything else
     *
     * @param string $message what you wanna log
     */
    ...
    public function parse(?string $text): string
    {
        ...
        $this->text = $text;
        $this->log('parse ' . (($this->is_sub_parser) ? 'as sub parser' : 'as master'));
        if (false === $this->is_sub_parser) $this->text = "\n" . $this->text . "\n";
    ...
    /**
     * @since 0.6.17 will output a p or br when necessary (to avoid #textnodes in final html output)
     */
                        ...
                        (null !== ($positions = $this->nextPartOfCommandOnSameLine($text, ($tag_sig = ']('), $pointer))
                            ...
                            if ($end < $next_EOL) {
                                ...
                                if (strlen($href) === 0) {
                                    ...
                                    echo (new Parser(true, $this->logger))->parse(substr($text, $pointer, $tag_sig_position - $pointer));
                                    echo '</a>';
                                }
                                $end++;
                                $this->pointer = $end;
    ...
    /**
     * Go hunt for the next command
     *
     * @return array|null command as array, or null when the end is reached
     */
    ...
    private function getLinkById(string $id, string $link_text): string
    {
        ...
        if (false === $pos) {
            ...
            $link_text = (new Parser(true, $this->logger))->parse($link_text);
            if (isset($title)) {
