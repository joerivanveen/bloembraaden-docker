...
class Base
{
    // NOTE we track whether THIS object has a message or error, but we save and retrieve them all through env (Help takes care of that)
    private bool $has_error = false, $has_message = false;
     ...
     *
     * @param string $message String Localized message that will be displayed to the client
     * @param string $level String default 'log', also possible: 'warn', 'error'
     */
    ...
    /**
     * @return bool Whether the object has a or multiple messages for the client
     */
    ...
    /**
     * @return \Exception the last error that occurred
     */
    ...
    public function handleErrorAndStop($e, string $message_for_frontend = 'A program error occurred')
    {
        ...
        $s = str_replace(array("\n", "\r", "\t"), '',
            strip_tags($message_for_frontend));
    ...
    /**
     * Sends a redirect_uri of 307 temporary redirect header, use it for items that are not online
     * @param string|null $slug
     * @since 0.7.6 elements that are not online will generate a user-specifiable error akin to 404
     */
    public function handleNotFoundAndStop(?string $slug)
    {
        if (null !== $slug) {
