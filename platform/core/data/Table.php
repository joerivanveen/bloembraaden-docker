...
class Type extends Base
{
    ...
    public function __construct(string $type_name)
    {
        ...
        if (isset($this->peatcms_types[$type_name])) {
            ...
            // array_search return false or the first key that is found that has this value (needle)
...
class Table extends Base
{
    ...
    public function formatColumnsAndData(array $cols, bool $for_update = false): array
    {
        ...
        if (! $for_update) {
            ...
            if (! array_key_exists('instance_id', $cols)) {
            ...
            if ($this->info->hasStandardColumns()) {
                if (! array_key_exists('deleted', $cols)) {
                    $cols['deleted'] = false;
            ...
            /*} else { // there is no else, you can't update columns on the fly, e.g. menu admin that are specific to an instance etc.
                }*/
        }
        ...
        foreach ($cols as $column_name => $value) {
            if ($col = $this->getInfo()->getColumnByName($column_name)) {
                ...
                if ($for_update and ! $this->isValid($col, $value)) {
                    $r['discarded'][] = $column_name;
                    continue;
                }
                $like = false;
                $lower = false;
                $r['columns'][] = $column_name;
                if (in_array($col->getType(), array('boolean', 'smallint', 'integer', 'bigint'))) {
                    $value = (int)$value;
                    $r['values'][] = $value; // booleans must be 0 or 1, or postgresql will reject them
                } else {
                    $r['values'][] = $value;
                    if (! $for_update) {
                ...
                $r['parameterized'][] = $column_name . $like . $lower;
            } else {
                $r['discarded'][] = $column_name;
            }
        }
        if (isset($r['discarded']) and count($r['discarded']) > 0) {
            $this->addError('Discarded columns in statement: ' . var_export($r['discarded'], true));
        }
...
class Column
{
    private array $constrained_values;
    ...
    public function __construct(string $name, \stdClass $column)
    {
        ...
        $this->default = (strpos($column->default, 'nextval') === false) ? $column->default : 'serial';
        $this->nullable = (strpos($column->nullable, 'NO') === 0) ? false : true;
        $this->editable = (boolean)$column->editable;
        $this->length = $column->length; // for character type columns the maximum number of characters
        if ($name === 'template_id') { // TODO setup the template constraint
            $this->constrained_values = array('file', 'page', 'search');
        }
    }
