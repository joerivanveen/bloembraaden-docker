...
class DB extends Base
{
    ...
    public function fetchPropertiesRowSuggestions(string $src): \stdClass
    {
        ...
        $statement = $this->conn->prepare('SELECT p.property_id, pv.property_value_id, CONCAT(p.title, \': \', v.title) title, 
v.slug, (v.online AND p.online) online FROM cms_property p INNER JOIN cms_property_x_property_value pv
...
pv.deleted = FALSE AND p.deleted = FALSE AND v.deleted = FALSE AND p.instance_id = ' . Setup::$instance_id .
     ...
     * @since 0.5.15
     */
    public function fetchElementRowsWhereIn(Type $type, string $column_name, array $in, bool $exclude = false, int $limit = 1000): array
    {
        if (count($in) === 0) return array();
        $info = $this->getTableInfo($type->tableName());
        $sorting = '';
        if ($info->getColumnByName('date_published')) {
            $sorting = 'ORDER BY date_published DESC';
        } elseif ($info->getColumnByName('date_popvote')) {
            $sorting = 'ORDER BY date_popvote DESC';
        } elseif ($info->getColumnByName('o')) {
            $sorting = 'ORDER BY o ASC';
        }
        if ($info->getColumnByName($column_name)) {
            // TODO when admin chosen ONLINE value should be taken into account
            $and_where_online = (false === ADMIN) ? ' AND el.online = TRUE' : '';
            $statement = $this->conn->prepare('SELECT el.*, ' .
                $info->getIdColumn() . ' AS id, \'' . $info->getTableName() . '\' AS table_name FROM ' . $type->tableName() .
    ...
    public function fetchPropertyRowsLinked(Type $type, int $id): array
    {
        ...
        $statement = $this->conn->prepare(
            'SELECT ' . $type->typeName() . '_x_properties_id x_value_id, x.o, x.x_value, ' .
            'p.slug property_slug, p.title property_title, v.slug, v.title, (p.online AND v.online) online, ' .
            'EXISTS(SELECT 1 FROM cms_variant_x_properties ' .
...
//var_dump(str_replace(':id', $id, $statement->queryString));
     ...
     * @since 0.7.3
     */
    public function fetchInstagramMediaForFeed(int $instance_id, ?string $username, ?string $hashtag, int $limit): array
    {
    ...
    public function fetchPaymentStatuses(): array
    {
        $statement = $this->conn->prepare('SELECT u.*, o.order_number FROM _payment_status_update u LEFT OUTER JOIN _order o ON u.order_id = o.order_id WHERE u.instance_id = ' .
            Setup::$instance_id . ' ORDER BY date_created DESC, date_processed DESC;');
        $statement->execute();
        $rows = $statement->fetchAll();
        $statement = null;
    ...
    public function fetchSession(string $token): ?\stdClass
    {
        ...
        if (!$row = $this->fetchRow('_session', array(
            ...
            'session_id',
            'ip_address',
        ), array('token' => $token, 'instance_id' => null))) { // remove instance_id again when it's save
        ...
        foreach ($rows as $key => $row) {
            $menus[$row->slug] = array(
                '__menu__' => array( // menu is a complex tag, hence you need this as an array
                    '__item__' => $this->fetchMenuItems($row->menu_id)
                ),
                'title' => $row->title,
                'slug' => $row->slug,
            );
    ...
    public function jobCleanTemplateFolder(): string
    {
        ...
        if (($files = scandir(CORE . 'presentation/templates/'))) {
            $files = array_diff($files, array('..', '.'));
            foreach ($files as $index => $file_name) {
                if (false === isset($ids[intval(explode('.', $file_name)[0])])) {
                    unlink(CORE . 'presentation/templates/' . $file_name);
     ...
     *
     * @param int $session_id
     * @param string $name te name of the var
    ...
    private function fetchRows(string $table_name, array $columns, array $where = array(), bool $single = false)
    {
        ...
        if (count($columns) !== 0) {
            if ($columns[0] === '*') { // get all the columns
                $columns = $table->getInfo()->getColumnNames();
            }
            $columns = $table->getColumnsByName($columns);
        } else {
            $columns = array('names' => array());
        }
        $columns['names'][] = $table->getInfo()->getIdColumn() . ' AS id'; // always get the id of the table, as ->id
    ...
    private function updateRowAndReturnSuccess(string $table_name, array $data, $key): bool
    {
        ...
        $statement = $this->conn->prepare(
            ...
            ' . ($table->getInfo()->hasStandardColumns() ? ', date_updated = NOW()' : '') . '
    ...
    private function deleteRowAndReturnSuccess(string $table_name, $key): bool
    {
        ...
        // delete the row
        if ($table->getInfo()->hasStandardColumns()) { // if the deleted column exists, use that
            $statement = $this->conn->prepare('UPDATE ' . $table_name . ' SET date_updated = NOW(), deleted = TRUE WHERE ' .
                $table->getInfo()->getPrimaryKeyColumn()->getName() . ' = ?');
        } else {
    ...
    private function fetchTableInfo(string $table_name): TableInfo
    {
        ...
        $file_name = CORE . '/data/cache/' . Setup::$VERSION . $table_name . '.info.serialized';
        ...
        if (false === file_exists($file_name)) file_put_contents($file_name, serialize($info), 2);
    ...
    private function copyRowToHistory(Table $table, $key): ?\stdClass
    {
        ...
        if (in_array($table_name = $info->getTableName(), $this->tables_without_history)) return null;
        // ok continue
        $row = $this->fetchRow($table_name, array('*'), array(
            $info->getPrimaryKeyColumn()->getName() => $key,
        ...
        if ($row === null) {
            $this->addError(sprintf('->copyRowToHistory() could not get row from %1$s with %2$s = %3$s',
                $table_name, $info->getPrimaryKeyColumn()->getName(), (string)$key));

            return null;
        }
        // 2) insert the row into history database
        try {
            $sql = 'INSERT INTO ' . $table_name . ' (' . implode(', ', $info->getColumnNames()) . ') VALUES (';
            $values = array();
            $parameters = array();
            foreach ($info->getColumnNames() as $key => $column_name) {
