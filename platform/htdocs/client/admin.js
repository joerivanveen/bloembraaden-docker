...
PEATCMS_actor.prototype.create_as_numeric = function (column) {
    ...
    if (this.pretty_id_column_names.includes(column_name)) {
        ...
        option.value = '0';
        option.hidden = 'hidden'; // hidden option is only shown in the list when no other option is selected
        el.options[0] = option;
        ...
        NAV.ajax(selectlist_actions[column_name], {for: this.parent_PEATCMS_element.state.type}, function (json) {
            ...
            if (self.server_value === 0 && el.options[1]) {
...
PEATCMS_actor.prototype.create_as_file_upload = function (column) { // create a drop area with handlers that save the dropped file and update server side
    ...
    el.classList.add('drop_area', 'file');
    ...
    self.process = function () {
        var source = new EventSource('/__action__/process_file/sse:true/slug:' + self.parent_PEATCMS_element.state.slug);
...
PEATCMS_actor.prototype.hasChanged = function () {
    ...
    if (typeof DOMElement.selectedIndex !== 'undefined') { // select lists always return option value as string
        return (this.server_value.toString() !== DOMElement.options[DOMElement.selectedIndex].value);
    }
...
var PEATCMS_x_value = function (row, parent_element) { // contains property / property_value x_value triple cross table
    ...
    new PEATCMS_column_updater(input);
...
var PEATCMS_column_updater = function (DOMElement) {
    //super();
    ...
    if (DOMElement.hasAttribute('id') && DOMElement.getAttribute('id') === 'peatcms_publish') {
        // TODO test this incl. reflectChanged, Firefox says this is type 'submit' even though it's a button
        ...
        var value, i, len; // set selected option first
        ...
        DOMElement.addEventListener('click', function () {
            var msg = DOMElement.getAttribute('data-confirm'),
...
PEATCMS_column_updater.prototype.keydown = function (event) {
    ...
    if (event.key === 'Tab' && this.DOMElement.tagName === 'TEXTAREA') {
        ...
        event.stopPropagation();
        event.returnValue = '';
        insertAtCaret(this.DOMElement, '    '); // insert four spaces in stead of a tab
        return false;
    }
}
...
var PEATCMS_admin = function () {
    var self = this,
    nodes, node, len, element_name, i, hidden_cells;
    this.elements = {};
    this.instance = null;
    this.orders = {};
    ...
    for (i = 0, len = nodes.length; i < len; ++i) {
        ...
        var cell = node.parentNode; // TODO this is true now, but rework to find '.cell' as a parentNode
        if (node.hasAttribute('data-element_name') && hidden_cells.hasOwnProperty(node.getAttribute('data-element_name'))) {
    ...
    document.addEventListener('peatcms.document_ready', function () {
        ...
        if ((el = document.getElementById('bloembraaden-css'))) {
            if (document.getElementById('PEATCMS_admin_page')) {
                ...
                if ((el = document.querySelector('[name="bloembraaden-default-css"]'))) {
                    PEATCMS.removeNode(el);
                }
            }
        }
        // automate status of it and set some other buttons
    ...
    window.addEventListener('keydown', function (event) {
        var els;
        // ctrl+, = toggle edit, ctrl+. = toggle console, ctrl+p = show / hide tools
        if (event.key === 'Control') {
            if ((els = document.querySelectorAll('[data-ctrl_key]'))) {
                els.forEach(function (el) {
                    var prev_element = el.previousElementSibling;
                    if (prev_element && prev_element.className === 'peatcms_ctrl_key_tip') return;
                    el.insertAdjacentHTML('afterbegin',
                        '<div class="peatcms_ctrl_key_tip">' +
                        el.getAttribute('data-ctrl_key') + '</div>'
                    );
                });
            }
        }
...
PEATCMS_admin.prototype.pollOrders = function () {
    /*NAV.ajax('/__admin__/__action__/orders',{},function(json){
        console.warn(json);
    });*/
    console.warn('pollOrders does nothing at the moment');
}
/**
...
PEATCMS_admin.prototype.edit = function (slug) {
    ...
    /*if (slug === el.state.slug) {
        el.edit(self.panels.get('sidebar').getDOMElement(), function () {
...
PEATCMS_admin.prototype.enhanceToggle = function (elements) {
    elements.forEach(function (el) {
        var toggler = el.parentNode.parentNode,
        ...
        if (toggler.hasAttribute('data-instance_id') &&
            parseInt(toggler.getAttribute('data-instance_id')) !== CMS_admin.instance.instance_id) {
            ...
            el.onclick = function () { // remove any other onclick handlers that might linger on the button
                var toggler = this.parentNode.parentNode,
...
function peatcms_admin_start() {
    ...
    if (typeof NAV === 'function') {
        ...
        document.addEventListener('peatcms.initialize', peatcms_admin_setup);
    }
}
...
function peatcms_admin_setup() {
    ...
    // update other buttons maybe
    document.addEventListener('peatcms.document_ready', function () {
        var input, inputs = document.querySelectorAll('.admin_order_search');
        // TODO using localstorage to remember open / close status, make that more generic than only here
        ...
        if ((input = document.getElementById('payment_link'))) {
