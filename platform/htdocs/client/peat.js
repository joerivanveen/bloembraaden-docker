...
function PeatStickyObject(stickyElement, upperElement, lowerElement, stickToElement, margins) {
    var self = this;
    if ('undefined' === typeof margins) margins = 20; // replace default which is not supported < ES6
    self.DOMElement = stickyElement;
    self.element_is_fixed = ('fixed' === window.getComputedStyle(stickyElement).getPropertyValue('position'));
    self.element_top = parseInt(window.getComputedStyle(stickyElement).getPropertyValue('top'));
    self.upper_element_margin = parseInt(window.getComputedStyle(upperElement).getPropertyValue('margin-bottom'));
    self.upper_el = upperElement;
    self.lower_el = lowerElement;
    self.stick_el = stickToElement;
...
PeatStickyObject.prototype.handleEvent = function () {
    var div = this.DOMElement,
        stick_el = this.stick_el,
        upper_el = this.upper_el,
        window_top = window.pageYOffset,
        lower_top = this.lower_el.offsetTop,
        sticker_top = stick_el.offsetTop + stick_el.getBoundingClientRect().height,
        upper_top = upper_el.offsetTop + upper_el.getBoundingClientRect().height + this.upper_element_margin,
        div_height = div.getBoundingClientRect().height + this.margins;
    if ((lower_top - window_top) < (sticker_top + div_height)) {
        div.classList.remove('stick');
        div.style.top = (this.element_is_fixed ? this.element_top : (lower_top - div_height)) + 'px';
    } else if (window_top - upper_top > -this.margins - sticker_top) {
        div.classList.add('stick');
        div.style.top = (this.margins + sticker_top) + 'px';
    } else {
        div.classList.remove('stick');
        div.style.top = '';
    }
...
document.addEventListener('peatcms.document_complete', function (e) {
    var wrappers = document.querySelectorAll('[data-peatcms_enhance_address]'),
        i, len, wrapper;
    for (i = 0, len = wrappers.length; i < len; ++i) {
        wrapper = wrappers[i]; // this is a container holding address elements you want to enhance
        wrapper.Address = new Address(wrapper);
        // addresses in wrappers can be present as sessionvars!
        if (false === wrapper.hasAttribute('id')) {
            console.error('Address wrapper element needs a unique id');
        } else {
            wrapper.Address.updateClientOnly(PEAT.getSessionVar(wrapper.id));
        }
    }
});
...
PEATCMS_element.prototype.setLinked = function (type, data) {
    ...
    // TODO while elements don't detect changes, render it here immediately
    this.render();
    //
    if (VERBOSE) {
...
PEATCMS_element.prototype.set = function (data) {
    ...
    for (column_name in data) {
        ...
        if ((column = this.getColumnByName(column_name)) && column.hasOwnProperty('actor')) {
            column.actor.set(data); // TODO highlight the update shortly
        }
    }
...
PEATCMS_ajax.prototype.fileUpload = function (callback, file, for_slug, element) {
    var xhr = this.getHTTPObject(), self = this, prgrs = document.createElement('div');
    // setup the element for progress feedback
    if (element !== null) {
        prgrs.className = 'progress';
        element.insertAdjacentElement('afterbegin', prgrs);
    } else {
...
PEATCMS_ajax.prototype.setUpProcess = function (xhr, on_done) {
    var self = this,
        data = {}, i, len, arr, obj;
    ...
    if (typeof on_done === 'function') {
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState === 4) {
                ...
                if (false === data.hasOwnProperty('html')) { // todo refactor to ‘is_template’ flag or something
...
PEATCMS_ajax.prototype.ajax = function (url, data, callback, method, headers) {
    ...
    // TODO use timestamp to distinguish between the same requests (e.g. search) where only the last one is relevant
    if (!data) {
        data = {json: true}
        ...
        if ((tags = this.progressive_tags[render_in_tag])) {
            for (i = 0, len = tags.length; i < len; ++i) {
                ...
                try {
                    ...
                    //PEAT.ajaxifyDOMElements(el.parentNode);
                    PEAT.ajaxifyDOMElements(parent_node);
                    // @since 0.5.15 produce an event with relevant details to catch
                    document.dispatchEvent(new CustomEvent('peatcms.progressive_ready', {
                        bubbles: true,
                        detail: {
                            placeholder_element: el, // to be DEPRECATED!
...
PEATCMS_template.prototype.renderOutput = function (out, template) {
    var break_reference = template.html,
        ...
        if (typeof output_object === 'object') {
            ...
            if (template.hasOwnProperty(tag_name)) {
                ...
                for (i = 0, len = template[tag_name].length; i < len; i++) {
                    ...
                    break_reference = sub_template.html;
                    ...
                    if (output_object.hasOwnProperty(0)) { // build rows if present
                        ...
                        for (temp_i = 0; temp_i < sub_template.__row__.length; temp_i++) {
                            ...
                            sub_template.html = sub_template.html.replace('{{__row__[' + temp_i + ']}}', build_rows);
                        }
                    }
...
PEATCMS_template.prototype.getComplexTagString = function (tag_name, html, offset) {
    ...
    if (!offset) offset = 0; // replacing default value which is not supporter < ES6
...
/*PEATCMS_template.prototype.prepare = function (html) {
    var template = {},
    ...
    while ((tag_name = this.findComplexTagName(html))) { // this takes care of the __row__ as well...
        if (!template.hasOwnProperty(tag_name)) template[tag_name] = [];
        if ((string = this.getComplexTagString(tag_name, html))) {
            number = template[tag_name].length;
            template[tag_name][number] = this.prepare(this.getInnerContent(string));
            html = html.replace(string, '{{' + tag_name + '[' + number + ']}}');
        } else {
            console.error('Complex tag error: ' + tag_name);
            break;
        }
    }
...
PEATCMS.prototype.render = function (element, callback) {// don't rely on element methods, it could have been serialized
    ...
    // set status
    if (this.document_status !== 'rendering') {
        this.document_status = 'rendering';
    ...
    if ((html = template.render(out))) {
        ...
        if (typeof CMS_admin === 'undefined') {
            ...
            document.querySelectorAll('[data-peatcms_handle="edit_current"]').forEach(function (el) {
                el.setAttribute('disabled', (element.isEditable()) ? '0' : '1');
            });
        }
        if (this.document_status !== 'interactive') {
            this.document_status = 'interactive';
        ...
        // emit event
        if (this.document_status !== 'loaded') this.document_status = 'ready'; // loaded could come before ready theoretically
        document.dispatchEvent(new CustomEvent('peatcms.document_ready', {
            bubbles: true,
            detail: {
                title: out.title,
...
PEATCMS.prototype.registerAssetLoad = function (which) {
    ...
    this.loderunner--;
    if (this.loderunner === 0 && this.document_status !== 'complete') {
        this.document_status = 'complete';
...
PEATCMS.prototype.swipifyDOMElement = function (el, cb_left, cb_right) {
...
PEATCMS.prototype.startUp = function () {
    var i, len, self = this;
    self.html_node = document.getElementsByTagName('html')[0];
    ...
    document.addEventListener('peatcms.document_ready', function (e) {
        var i, len, a, href,
            as = document.getElementsByTagName('a'), // faster than querySelectorAll()
            current_slug = e.detail.path + '|'; // the pipe character is to match wordboundary, so you match the entire slug
        for (i = 0, len = as.length; i < len; ++i) {
            a = as[i];
            href = decodeURI(a.href) + '|'; // match the exact slug, not when it’s a part of the link
            if (href.indexOf(current_slug) !== -1) {
    ...
    window.addEventListener('scroll', function () {
        ...
        if (window.pageYOffset > 4) { /* consider the page scrolled */
            self.html_node.setAttribute('data-peatcms-scrolled', '1');
        } else {
            self.html_node.removeAttribute('data-peatcms-scrolled');
        }
    });
}
...
PEATCMS.prototype.grabAttention = function (DOMElement) {
    DOMElement.classList.add('peatcms_attention_grabber');
    setTimeout(function (el) {
        el.classList.remove('peatcms_attention_grabber');
    }, 600, DOMElement);
}
...
PEATCMS.prototype.scrollTo = function (x, y) {
    if ('scrollBehavior' in document.documentElement.style) {
        window.scrollTo({
            left: x,
            top: y,
...
var PEATCMS_navigator = function (root) {
    ...
    this.went = null; // remember if we went somewhere todo remove after petitclos is updated
...
PEATCMS_navigator.prototype.maybeEdit = function (slug) {
    // if edit screen is open, fill it with the new element
    if (typeof CMS_admin === 'object') {
...
PEATCMS_navigator.prototype.admin_uncache_slug = function (path) {
    if (!path) path = this.getCurrentPath(); // replacing default value which is not supported < ES6
    this.ajax('/__action__/admin_uncache', {'path': path}, function (json) {
...
PEATCMS_navigator.prototype.refresh = function (path) {
    var self = this, globals = window.PEATCMS_globals;
    if (!path) path = this.getCurrentPath(); // replacing default value which is not supported < ES6
    if (window.history && window.history.pushState) {
        if (globals.hasOwnProperty('slug')) { // move the first page’s slug into the cache
            if (globals.slug.hasOwnProperty('__ref')) path = globals.slug.__ref;
            if (VERBOSE) console.log('Put ‘' + path + '’ into the cache from globals');
            // cache is built into PEATCMS_ajax
            this.cache({state: unpack_temp(window.PEATCMS_globals.slug)});
            delete window.PEATCMS_globals.slug;
        }
        new PEATCMS_element(path, function (el) {
            if (el === false) {
                ...
                PEAT.render(el, function (el) {
                    ...
                    //NAV.maybeEdit(); // you may be refreshing a page you are not editing, don’t switch over there
                });
            }
        });
    }
...
PEATCMS_navigator.prototype.submitForm = function (form) {
    var self = this, data = getFormData(form);
...
document.addEventListener('peatcms.document_ready', function () {
    ...
    var slideshow = document.querySelector('.peatcms-slideshow-wrapper');
    clearInterval(window.peatcms_slider_i);
    ...
    if (slideshow != null) { //make sure we don't run this script if the slideshow is not present
        ...
        if (typeof CMS_admin === 'undefined') {
            ...
            //slides = slideshow.getElementsByClassName('peatcms-slide-entry');
        }
        if (slideCount <= 0) {
        ...
        var moveToSlide = function (n) { // set up our slide navigation functionality
            ...
            window.addEventListener('resize', function () {
                var resizedSlideHeight = slides[currentSlide].clientHeight;
        ...
        slideHandlers.nextSlide('#peatcms-next-slide');
        slideHandlers.prevSlide('#peatcms-prev-slide');
...
// optional autoplay function
        window.peatcms_slider_i = setInterval(function () {
            nextSlide();
        }, 10000);
